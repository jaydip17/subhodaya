						...
						<div style="height:265px;border:1px solid #9FA0A0;padding-left:20px" id="left_news">
								...
								if(!empty($video_result)){
          				        ...
          				        foreach($video_result as $item){?>
          				 		<div style="padding:4px;height:82px;padding-top:7px;"><a href="<?=base_url();?>video/index/<?=$item->id?>/<?=$item->video_cat_id?>"><div id="cinemavideo_time"><?=$item->time?></div><img id="img" src="<?=base_url();?>assets/videos/image_preview/home_image<?=$item->id?>_thumb.jpg"></a></div>
							        		<div id="gall_botto_img" align="center"style="margin-left:2px;"><a href="<?=base_url();?>video/index/<?=$item->id?>/<?=$item->video_cat_id?>"><?=$item->name?></a></div>
