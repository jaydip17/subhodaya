									...
									<?php }?>
										...
										<?php foreach ($details as $row):?>
											<li style="color:#228BEE;line-height: 19px;" >
												<a href="<?=$link?><?=$row->id?>/<?php if(!empty($link_cate)){echo $link_cate;}else{echo $row->cat_id;}?>" style="color:#228BEE; font-size:11px; text-decoration:none;outline-style: none;">
												<?=$row->heading?>
												</a>
											</li>
											<?php endforeach;?>
										...
										<div style="color:#AEAEAE; font-size:11px; margin-top:0px" align="right"><a href="<?= (isset($more_link)) ? $more_link : '' ?>"><?= (isset($more)) ? $more : '' ?></a></div>
